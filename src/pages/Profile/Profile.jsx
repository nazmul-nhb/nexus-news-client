...
const Profile = () => {
	...
	const { isNexusUserLoading, data: nexusUser = {} } = useNexusUsers(
		['nexusUser', user?.email],
		user?.email
	);
	const now = moment();
		...
		<section className="mx-6 md:mx-10 py-2 md:py-8 p-2 md:px-4">
			...
			<div className="flex flex-col lg:flex-row gap-10 items-center mb-8 lg:mb-16">
								...
								<div className="flex flex-col lg:flex-row gap-2 items-center lg:items-start justify-center lg:justify-start my-4">
									...
									<PhotoProvider>
										<PhotoView src={user.photoURL}>
											<img
												className="user-name cursor-pointer border p-1 border-nexus-primary w-24 md:w-36 h-24 md:h-36"
												src={user.photoURL}
									...
									<div className="flex flex-col items-center lg:items-start justify-center lg:justify-start gap-3">
										...
										<h4 className="verification-status font-semibold flex items-center gap-1">
											{user.email}{' '}
											{user.emailVerified ? (
												<MdVerified className="text-nexus-primary" />
											) : (
								...
								<div>
									...
									{nexusUser?.updated_on && (
										<div className="flex flex-col items-center lg:flex-row gap-1 md:text-xl">
											...
											<h4>
												{moment(nexusUser?.updated_on).format(
													'MMMM DD, YYYY [at] hh:mm:ss A'
												)}
											</h4>
										</div>
										...
										<>
											<div
												className={`${
													...
													'text-red-600'
												} flex flex-col items-center lg:flex-row gap-1 md:text-xl`}
											>
											...
											>
												...
												<h4>
													{moment(nexusUser?.expires_on).format(
														'MMMM DD, YYYY [at] hh:mm:ss A'
													)}
												</h4>
											</div>
											<div
												className={`flex flex-col items-center lg:flex-row gap-1 md:text-xl`}
											>
												<h4 className="font-semibold">
													Last Transaction ID:
												</h4>
				...
				<form
					...
					className="w-full lg:w-2/5 flex flex-col gap-6 px-4 lg:px-8 py-4 lg:py-6 shadow-lg shadow-nexus-primary border border-nexus-primary rounded-md"
				>
					{/* Name */}
					...
					>
						{userLoading || imageUploading ? buttonLoader : 'Update Profile'}
					</button>
				</form>
			</div>
		</section>
