...
import { searchLoader } from '../../components/LoadingSpinners/Loaders';
...
const AllArticles = () => {
	...
	const { isLoading, data: allArticles } = useGetArticles(
		...
		`tag=${selectedTag?.value}&publisher=${
			selectedPublisher?.value
		}&search=${searchText}&sort=${sortByTime ? 'time_descending' : 'time_ascending'}`
	);
	...
	useEffect(() => {
		if (searchText && allArticles?.length > 0) {
			toast.success(
				`${allArticles?.length} ${
					allArticles?.length > 1 ? 'Matches' : 'Match'
				} Found!`
			);
		}
	...
	return (
		<section className="mx-6 md:mx-10 py-2 md:py-8 p-2 md:px-4">
			...
			<SectionHeader
				heading={'Explore All Our Articles'}
				subHeading={`Total: ${allArticles.length}  ${
					allArticles?.length > 1 ? 'Articles' : 'Article'
				}`}
			/>
			...
			<div className="grid md:grid-cols-2 lg:grid-cols-4 mx-auto gap-4 mb-8 text-sm">
				...
				<form
					...
					className="flex gap-2 items-center justify-start text-nexus-secondary"
				>
					<div className="flex gap-2 w-full items-center relative pl-2 bg-transparent rounded-lg border border-nexus-secondary">
						...
						<input
							...
							className="px-2 rounded-r-lg py-[9px] bg-transparent w-full border-l border-nexus-secondary focus:outline-0"
						...
						<div className="absolute right-0 flex gap-2">
							{searchText !== '' && (
								<button
									...
									className="text-2xl hover:text-nexus-primary transition-all duration-500"
								>
									<MdClear />
								</button>
							)}
							<button
								className="border py-[9px] px-4 rounded-r-lg font-bold tracking-wider border-nexus-secondary bg-nexus-secondary text-white hover:bg-white hover:text-nexus-secondary transition-all duration-700"
								type="submit"
							>
								Search
							</button>
				...
				<button
					...
					type="submit"
				>
					{sortByTime ? 'See Oldest Articles' : 'See Latest Articles'}
				</button>
			</div>
			<div className="">
				{searchText && isLoading ? (
					...
					<p className="flex items-center justify-center text-lg font-semibold text-red-700">
						Nothing Found!
					</p>
				) : (
					<ReactPlaceholder
