...
const MyArticles = () => {
	...
	const articlesWithSerial = userArticles?.map((article, index) => ({
		...article,
		serial: index + 1,
	}));
	...
	const articleColumns = [
		...
		},
		{
			...
			cell: (cell) => {
				return (
					<Link
						className="hover:text-nexus-primary font-semibold font-kreonSerif"
					...
					<div className="flex justify-between gap-1 items-center">
						...
						{status === 'Declined' && (
							<>
								...
								<button
									className="text-nexus-primary font-semibold"
									onClick={() => handleShowReason(decline_reason)}
								>
									Reason
								</button>
							</>
						...
						{!status && (
							<span className="flex items-center gap-0.5 text-nexus-secondary font-semibold">
		...
		},
		{
			...
			cell: (cell) => {
				return (
					<button
						className="text-center flex gap-0.5 items-center text-green-700 hover:text-nexus-primary font-semibold transition-all duration-50"
		...
		},
		{
			...
			cell: (cell) => {
				...
				return (
					<button
						className="text-center flex gap-0.5 items-center text-red-700 hover:text-nexus-primary font-semibold transition-all duration-500"
	...
	return (
		<section className="mx-6 md:mx-10 py-2 md:py-8 p-2 md:px-4">
			...
			{!isLoading && !userArticles.length ? (
				<p className="flex items-center justify-center text-red-700 text-lg font-semibold">
				...
				<ReactPlaceholder
					...
					ready={!isLoading}
				>
					<NexusTable data={articleData} columns={articleColumns} />
				</ReactPlaceholder>
			)}
			...
			{showReasonModal && (
				<dialog
					open
					className="w-[96%] xl:w-auto h-3/4 bg-opacity-95 p-12 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-transparent rounded-lg z-50 overflow-y-auto"
				>
					<div className="animate__animated animate__pulse bg-[#c5cce5f1] border shadow-lg h-full rounded-lg p-6 flex flex-col justify-around">
						<IoIosCloseCircle
							...
							className="absolute -top-4 -right-4 text-5xl text-red-700 hover:text-nexus-primary hover:opacity-80 transition-all duration-500 cursor-pointer"
							title="Close"
						/>

						<div className="flex flex-col items-center gap-4 h-3/4 p-1">
							<h3 className="animate__animated animate__headShake text-center text-red-700 font-kreonSerif font-bold max-[430px]:text-lg text-2xl mb-4 md:mb-6">
								Reason Your Article was Declined!
							</h3>
			...
			{showUpdateModal && (
				<dialog
					open
					className="w-full h-full bg-opacity-95 p-11 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-transparent rounded-lg z-50 overflow-y-auto"
				>
