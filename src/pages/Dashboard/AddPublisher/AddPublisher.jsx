...
const AddPublisher = () => {
	...
	return (
		<section className="w-full flex flex-col items-center gap-4 overflow-x-auto pt-6 pb-12">
			...
			<form
				onSubmit={handleSubmit(handleAddPublisher)}
				className="w-full md:w-1/4 md:flex-grow-0 flex flex-col gap-6 px-4 lg:px-8 py-4 lg:py-6 shadow-lg shadow-nexus-secondary border border-nexus-secondary rounded-md"
			>
				{/* Publisher Name */}
				<div className="flex flex-col gap-3">
					<label
						className="font-medium text-center text-nexus-primary"
						htmlFor="publisher"
					>
						Add A Publisher *
					</label>
				...
				<button className={`${buttonNormal} justify-center`}>
					{imageUploading ? buttonLoader : 'Add Publisher'}
				</button>
			</form>
			<div className="border-t w-full my-10"></div>
			<SectionHeader
				heading={'List of Publishers'}
				subHeading={`Total Publishers: ${publishers.length}`}
			/>
			...
			>
				<div className="grid md:grid-cols-3 gap-8 w-3/4">
					{publishers?.map((pub, index) => (
						<div
							className="relative flex flex-col justify-around items-center gap-4 p-3 border border-nexus-primary rounded-md shadow-lg shadow-nexus-primary bg-nexusBG"
							key={index}
						>
							...
							<h4 className="flex items-center gap-2">
								<MdManageHistory />
								{moment(pub.added_on).format('dddd, MMMM DD, YYYY')}
							</h4>
							<button
								onClick={() =>
									handleDeletePublisher(pub._id, pub.publisher)
								}
								className="absolute z-30 right-0.5 top-0.5 text-4xl text-red-600"
							>
								<MdDeleteForever />
							</button>
						</div>
